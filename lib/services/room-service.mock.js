// Mock implementation of the RoomService interface (see room-service.js).
//
...
// real-time sync). It is one interchangeable implementation behind the
// service seam — swap in an HTTP/websocket implementation later without
...
async function createRoom({ problem, language, starterCode, sessionId }) {
  const rooms = readAll()
...
async function joinRoom(code, { name, sessionId }) {
...
async function updateCode(code, nextCode, sessionId) {
  code = (code || '').toUpperCase()
  const rooms = readAll()
  const room = normalize(rooms[code])
...
async function resetCode(code, sessionId) {
  code = (code || '').toUpperCase()
  const rooms = readAll()
  const room = normalize(rooms[code])
  if (!room) return delay({ ok: false, error: 'not_found' })
...
async function endRoom(code, sessionId) {
  code = (code || '').toUpperCase()
  const rooms = readAll()
  const room = normalize(rooms[code])
  if (!room) return delay({ ok: false, error: 'not_found' })
...
async function broadcastExecution(code, result) {
  code = (code || '').toUpperCase()
  const rooms = readAll()
  const room = normalize(rooms[code])
  if (!room || room.status !== 'active') return { ok: false }
...
// The mock RoomService — one implementation of the interface in room-service.js.
